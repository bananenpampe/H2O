...
class RascalineAtomisticDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for frame in frames:
            self.dimer_type.append(frame.info["label"])
        
        """
        for frame in frames:
            self.energy_A.append(frame.info["energy_A"])
            self.energy_B.append(frame.info["energy_B"])
        """
        ...
        self.energy_A = [ase_to_tensormap(frame, "energy_A") for frame in frames]
        self.energy_B = [ase_to_tensormap(frame, "energy_B") for frame in frames]
        self.only_interaction = []

        if remove_DFT:
            #TODO: fix this
            for E_tot, E_A, E_B in zip(self.properties, self.energy_A, self.energy_B):
                
                tmp_block = TensorBlock(values= E_tot.block(0).values - (E_A.block(0).values + E_B.block(0).values), 
                                        properties=Labels.range(energy_key, 
                                              E_tot.block(0).values.shape[-1]), 
                                              components=[], 
                                              samples=E_tot.block(0).samples)
                
                tmp_map = TensorMap(E_tot.keys, [tmp_block])
                self.only_interaction.append(tmp_map)
    ...
    def __getitem__(self, idx: int):
        ...
        return feats, self.properties[idx], self.frames[idx], self.dimer_type[idx], self.energy_A[idx], self.energy_B[idx]
