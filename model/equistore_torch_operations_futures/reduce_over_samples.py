...
from equistore.operations import _dispatch
...
def _reduce_over_samples_block(
    ...
    if len(block.samples) == 0:
        ...
        samples_label = Labels(
            remaining_samples,
            torch.zeros((0, len(remaining_samples))),
        )
    ...
    if len(remaining_samples) == 0:
        ...
        #print(type(remaining_samples))
        #print(type(remaining_samples[0]))

        #print(type(new_samples))

        samples_label = Labels(
            remaining_samples,
            torch.Tensor(new_samples).to(dtype=torch.int32),
        )
