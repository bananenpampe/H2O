...
def union_join(
    ...
    for tensor_i, tensor in enumerate(tensors):
        ...
        new_keys = Labels(names=tensor.keys.names, values=torch.vstack([tensor.keys.values, missing_key_values]))

        for _ in missing_key_values.flatten():
            new_blocks.append(empty_like_block(tensor[0]))

        tensors[tensor_i] = TensorMap(keys=new_keys, blocks=new_blocks)
        
    join(tensors, axis="samples")
    ...
    keys = Labels(names=keys_names, values=torch.tensor(keys_values))
