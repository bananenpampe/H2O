...
class ExtractCalculator(Calculator):
    """Calculator to extract information from Atoms info section."""
    def __init__(self, energy: str, forces: str):
        super().__init__()
        self.energy = energy
        self.forces = forces
        
    def get_forces(self, atoms):
        return atoms.arrays[self.forces]
        
    def get_potential_energy(self, atoms, force_consistent=False, apply_constraint=True):
        return atoms.info[self.energy]

    def get_potential_energies(self, atoms):
        return self.get_potential_energy(atoms)

if __name__ == "__main__":
    frames = load_PBE0_TS()[:100]

    datamodule = LitDataModule(frames, batch_size=16, shuffle=False, verbose=True)
